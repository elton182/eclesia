...
- [ ] Seed cria papéis canônicos: `admin-tenant`, `gestor-site`, `admin-igreja`, `cadastros-usuarios`, `cadastros-equipes`, `cadastros-casais`, `cadastros-eventos`, `cadastros-calendario`, `lider-equipe`.
- [ ] Seed cria permissões de tela: `telas.usuarios`, `telas.equipes`, `telas.casais`, `telas.eventos`, `telas.calendario` (+ gestão users/roles, `ecc.*`, `calendario.*`).
